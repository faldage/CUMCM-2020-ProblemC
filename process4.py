...
while True:
    ...
    for i in range(0, n):
        ...
        if not ssl.__contains__(getsl):
            sumbad += getcount
        loc += 2
    ans.cell(num - 122, 1).value = res[0]
    ans.cell(num - 122, 2).value = float(fx[_type])
    ans.cell(num - 122, 3).value = (float(sumbad))/(float(sum))


while True:
    lines = f2_out.readline() # 整行读取数据
    if not lines:
        break
        pass
    res = lines.split(' ')
    ...
    for i in range(0, n):
        ...
        if not ssl.__contains__(getsl):
            sumbad += getcount
        loc += 2
    ans.cell(num - 122, 4).value = (float(sumbad))/(float(sum))


new.save(r'附件2进项销项不合格率以及行业风险.xlsx')
